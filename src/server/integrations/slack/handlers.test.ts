...
describe('slack handlers', () => {
  ...
  it('starts review-only run when @kanvy yes is sent after pending review confirmation', async () => {
      ...
      .mockResolvedValueOnce({
        ...
        data: {
          pendingReviewStart: {
            ...
            draftContext: 'Please focus on whether prior findings were fully addressed.'
          }
        },
    ...
    expect(repoBoard.createTask).toHaveBeenCalledTimes(1);
    expect(repoBoard.createTask).toHaveBeenCalledWith(expect.objectContaining({
      taskPrompt: expect.stringContaining('Operator context: Please focus on whether prior findings were fully addressed.')
    }));
    expect(repoBoard.startRun).toHaveBeenCalledTimes(1);
    expect(runOrchestratorMocks.scheduleRunJob).toHaveBeenCalledWith(
      expect.anything(),
  ...
  it('captures inline context in review rerun request when an existing review task is found', async () => {
    const repoBoard = makeRepoBoard({ taskId: 'task_review_inline_context', runId: 'run_review_inline_context' });
    (repoBoard as unknown as {
    ...
    const rawBody = JSON.stringify({
      type: 'event_callback',
      event_id: 'EvReviewInlineContext',
      team_id: 'team_one',
      authed_users: ['U0AJQ0GPJQL'],
      event: {
        type: 'message',
        ...
        ts: '1772728674.907969',
        user: 'U02MYQW90MT',
        text: '<@U0AJQ0GPJQL> review 12080 - it seems your initial comments were addressed',
        channel_type: 'channel'
      }
    });
    const signature = await buildSlackSignature('secret', nowTs, rawBody);
    ...
    expect(tenantAuthDbMocks.upsertSlackIntakeSession).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        status: 'active',
        data: expect.objectContaining({
          pendingReviewRerun: expect.objectContaining({
            repoId: 'repo_checkout',
            reviewNumber: 12080,
            reviewProvider: 'gitlab',
            draftContext: 'it seems your initial comments were addressed'
          })
        })
      })
    );
  ...
  it('appends rerun context instead of replacing existing draft context', async () => {
    const repoBoard = makeRepoBoard({ taskId: 'task_review_rerun_append', runId: 'run_review_rerun_append' });
    const boardIndex = makeBoardIndex([
      {
        repoId: 'repo_checkout',
        slug: 'engineering/frontend/frontend',
    ...
    tenantAuthDbMocks.getSlackIntakeSession.mockResolvedValue({
      ...
      data: {
        pendingReviewRerun: {
          ...
          reviewProvider: 'gitlab',
          reviewUrl: 'https://gitlab.rechargeapps.net/engineering/frontend/frontend/-/merge_requests/12080',
          draftContext: 'already captured context'
        }
      },
      lastActivityAt: new Date().toISOString(),
      createdAt: '2026-01-01T00:00:00.000Z',
    ...
    const body = JSON.stringify({
      ...
      event: {
        ...
        ts: '1772728736.734809',
        user: 'U02MYQW90MT',
        text: '<@U0AJQ0GPJQL> also verify bundle filtering changed',
        channel_type: 'channel'
      }
    });
    const signature = await buildSlackSignature('secret', nowTs, body);
    const request = new Request('https://example.test/api/integrations/slack/events', {
      method: 'POST',
      headers: slackHeaders(nowTs, signature),
      body
    ...
    expect(tenantAuthDbMocks.upsertSlackIntakeSession).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        data: expect.objectContaining({
          pendingReviewRerun: expect.objectContaining({
            draftContext: 'already captured context\n\nalso verify bundle filtering changed'
          })
        })
      })
    );
  ...
  it('routes @kanvy Jira mention through Jira confirmation flow', async () => {
    const env = makeEnv('secret');
    await env.SECRETS_KV.put('slack/bot-token', 'xoxb-test');
    fetchIssue.mockResolvedValueOnce({
      issueKey: 'AFCP-3013',
      title: 'Checkout fails on retries',
      body: 'Intermittent retry path fails with stale state.',
      url: 'https://jira.test.com/browse/AFCP-3013'
    });
    tenantAuthDbMocks.listJiraProjectRepoMappingsByProject.mockResolvedValue([
      { jiraProjectKey: 'AFCP', repoId: 'repo_alpha', priority: 0, active: true, id: 'm1', tenantId: 'tenant_local', createdAt: '', updatedAt: '' }
    ]);
    let llmCall = 0;
    fetchSpy.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (url.includes('https://slack.com/api/conversations.replies')) {
        return new Response(JSON.stringify({
          ok: true,
          messages: [
            { ts: '1672531200.1234', user: 'U1', text: '<@U_KANVY> fix AFCP-3013' }
          ]
        }), { status: 200 });
      }
      if (url.includes('/v1/chat/completions')) {
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                repoHint: '',
                repoId: '',
                taskTitle: 'Fix AFCP-3013 login edge case',
                taskPrompt: 'Implement and validate the AFCP-3013 fix.',
                acceptanceCriteria: ['AFCP-3013 is fixed'],
                missingFields: [],
                clarifyingQuestion: ''
              })
            }
    ...
    const response = await handleSlackEvents(request, env);
    expect(response.status).toBe(200);
    expect(fetchIssue).toHaveBeenCalledWith('AFCP-3013', 'team_one');
    expect(tenantAuthDbMocks.upsertSlackIntakeSession).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        tenantId: 'team_one',
  ...
  it('accepts plain thread replies without @kanvy when intake session is active', async () => {
    const repoBoard = makeRepoBoard({ taskId: 'task_plain_yes', runId: 'run_plain_yes' });
    const env = makeEnv('secret', repoBoard);
    await env.SECRETS_KV.put('slack/bot-token', 'xoxb-test');
    tenantAuthDbMocks.getSlackIntakeSession.mockResolvedValue({
      id: 'intake_plain_yes',
  ...
  it('accepts plain numeric repo choice replies without @kanvy in active intake threads', async () => {
    const env = makeEnv('secret');
    await env.SECRETS_KV.put('slack/bot-token', 'xoxb-test');
    tenantAuthDbMocks.getSlackIntakeSession.mockResolvedValue({
      id: 'intake_plain_number',
      tenantId: 'team_one',
      channelId: 'C123',
      threadTs: '1672531200.1234',
      status: 'active',
      ...
      data: {
        intent: 'create_task',
        taskTitle: 'Fix flaky checkout',
        taskPrompt: 'Fix flaky checkout test.',
        missingFields: ['repo'],
        repoChoices: ['repo_alpha', 'repo_beta', 'repo_gamma']
      },
      lastActivityAt: new Date().toISOString(),
      createdAt: '2026-01-01T00:00:00.000Z',
      updatedAt: '2026-01-01T00:00:00.000Z'
    ...
    fetchSpy.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('https://slack.com/api/conversations.replies')) {
        return new Response(JSON.stringify({
          ok: true,
          messages: [
            { ts: '1672531200.1230', user: 'U1', text: '<@U_KANVY> fix flaky checkout' },
            { ts: '1672531200.1243', user: 'U1', text: '3' }
          ]
        }), { status: 200 });
      }
      if (url.includes('/v1/chat/completions')) {
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                taskTitle: 'Fix flaky checkout',
                taskPrompt: 'Fix flaky checkout test.',
                acceptanceCriteria: ['Checkout test passes consistently'],
                missingFields: [],
                clarifyingQuestion: ''
              })
            }
